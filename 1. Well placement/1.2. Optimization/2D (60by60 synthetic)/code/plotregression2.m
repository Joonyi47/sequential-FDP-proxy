...
%  This code is the same for all Transfer Functions.
  ...
  if nargin == 0
    ...
    if nargout > 0
      out1 = fig;
    elseif ~isempty(fig)
      figure(fig, 'Units', 'centimeters', 'Position', [1 1 6 6]);
    end
    return;
  end
  in1 = varargin{1};
  if ischar(in1)
    ...
    set(fig, 'Units', 'centimeters', 'Position', [1 1 6 6]);
    set(fig,'Visible','on');
    drawnow;
    if nargout > 0, out1 = fig; end
  end
...
%           xlabel(a,'True NPV ($)',plotData.titleStyle{:}, 'FontName', 'Times New Roman');
          xlabel(a,'True NPV ($)','FontName', 'Arial');
          hold on
          plotData.axes(i) = a;
          ...
          plotData.dataPoints(i) = plot([NaN NaN],[NaN NaN],'ok','LineWidth',0.5, 'MarkerSize', 5);
