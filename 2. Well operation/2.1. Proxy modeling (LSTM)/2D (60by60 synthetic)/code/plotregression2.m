...
%  This code is the same for all Transfer Functions.
  ...
  if nargin == 0
    ...
    if nargout > 0
      out1 = fig;
    elseif ~isempty(fig)
      figure(fig, 'Units', 'centimeters', 'Position', [1 1 8.3 8.3]);
    end
    return;
  end
  in1 = varargin{1};
  if ischar(in1)
    ...
    set(fig, 'Units', 'centimeters', 'Position', [1 1 10 10]);
    set(fig,'Visible','on');
    drawnow;
    if nargout > 0, out1 = fig; end
  end
...
%           xlabel(a,'True NPV ($)',plotData.titleStyle{:}, 'FontName', 'Times New Roman');
          xlabel(a,'True NPV ($)','FontName', 'Arial', 'FontSize', 16);
          hold on
          plotData.axes(i) = a;

          plotData.eqLine(i) = plot([NaN NaN],[NaN NaN],'--k','LineWidth',1.5);
          color = colors{rem(i-1,length(colors))+1};
          plotData.regLine(i) = plot([NaN NaN],[NaN NaN],'LineWidth',1.5,'Color',color);
          plotData.dataPoints(i) = plot([NaN NaN],[NaN NaN],'ok','LineWidth',1, 'MarkerSize', 10);
          [hleg, hobj, hout, mout] = legend([plotData.dataPoints(i),plotData.regLine(i),plotData.eqLine(i)], ...
            {'Data','Regression line','Unit slope line'},'Location','NorthWest', 'FontSize', 12, 'FontName', 'Arial');
%         hobj(2).Children.MarkerSize = 10;
  ...
  for i=1:numSignals
    ...
    ax = gca;
    aa = ax.XTick;
    ax.XTick = [aa(1) aa(end)];
    ax.YTick = [aa(1) aa(end)];
...
%         plotData.titleStyle{:}, 'FontName', 'Times New Roman');
    title([name 'R^{2}=' num2str(mdl.Rsquared.Ordinary,'%0.3f')],plotData.titleStyle{:}, 'FontName', 'Arial', 'FontSize', 16);
