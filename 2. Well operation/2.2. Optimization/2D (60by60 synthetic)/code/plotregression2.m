...
%  This code is the same for all Transfer Functions.
  ...
  if ischar(in1)
    ...
    set(fig, 'Units', 'centimeters', 'Position', [1 1 8.3 8.3]);
    set(fig,'Visible','on');
    drawnow;
    if nargout > 0, out1 = fig; end
  end
...
function plotData = update_plot(param,fig,plotData,tt,yy,names)
    ...
    for plotrow=1:plotrows
      for plotcol=1:plotcols
        ...
        if (i<=numSignals)
          ...
          xlabel(a,'True NPV ($)',plotData.titleStyle{:}, 'FontName', 'Times New Roman', 'FontWeight', 'normal');
          hold on
          plotData.axes(i) = a;
          ...
          plotData.dataPoints(i) = plot([NaN NaN],[NaN NaN],'ok','LineWidth',1);
          legend([plotData.dataPoints(i),plotData.regLine(i),plotData.eqLine(i)], ...
            {'Data','Regression line','Unit slope line'},'Location','NorthWest');
  ...
  for i=1:numSignals
    ...
    set(gca,'XLim',lim);
    set(gca,'YLim',lim);
%     set(gca,'XLim',[2.655e+8 2.665e+8]);
%     set(gca,'YLim',[2.645e+8 2.665e+8]);
    axis('square')

    ylabel('Predicted NPV ($)',...
      plotData.titleStyle{:}, 'FontName', 'Times New Roman', 'FontWeight', 'normal');
    title([name 'R^{2}=' num2str(mdl.Rsquared.Ordinary,3)],plotData.titleStyle{:});
  end
  drawnow
end
